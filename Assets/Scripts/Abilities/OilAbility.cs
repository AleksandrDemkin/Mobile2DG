...
namespace Abilities
{
    public class OilAbility : IAbility
    {
        ...
        {
            _viewPrefab = ResourceLoader.LoadObject<Rigidbody2D>(new ResourcePath {PathResource =
                viewPath});
            if (_viewPrefab == null) throw new InvalidOperationException($"{nameof(OilAbility)} view requires {nameof(Rigidbody2D)} component!");
